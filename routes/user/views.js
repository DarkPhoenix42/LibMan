...
const view_checkin = async (req, res) => {
    ...
    try {
        const query =
            'SELECT t.*, b.title FROM transactions t JOIN books b ON t.book_id = b.book_id WHERE t.user_id = ? AND t.type = "checkout" AND t.status = "accepted"'
...
const view_history = async (req, res) => {
    ...
    try {
        const query =
            'SELECT t.*, b.title FROM transactions t JOIN books b ON t.book_id = b.book_id WHERE t.user_id = ?'
        results = await execute_query(query, [user_id])
